#include "wing_navigator/GetDouble.h"
...
public:
  ...
  virtual void Load(sensors::SensorPtr sensor, sdf::ElementPtr sdf) {
    ...
    this->serviceName =
        "/" + this->camera->Name() + "/" + pluginName + "/set_camera_zoom";
    this->getZoomName =
        "/" + this->camera->Name() + "/" + pluginName + "/get_camera_zoom";
    ...
    gzmsg << "CameraZoomPlugin loaded with parameters:\n"
          ...
          << "  Set Zoom Service: " << this->serviceName << "\n"
          << "  Get Zoom Service: " << this->getZoomName << "\n";
    ...
    this->rosService = this->rosNode->advertiseService(
        this->serviceName, &CameraZoomPlugin::SetZoom, this);
    this->getZoomService = this->rosNode->advertiseService(
        this->getZoomName, &CameraZoomPlugin::GetZoom, this);
...
private:
  ...
  bool GetZoom(wing_navigator::GetDouble::Request &req,
               wing_navigator::GetDouble::Response &res) {

    res.data = this->currentZoomFactor;
    return true;
  }
  ...
  ros::ServiceServer getZoomService;

  std::string serviceName;
  std::string getZoomName;
